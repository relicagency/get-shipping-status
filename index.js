...
let getCustomField = (contact, name) => _get(_get(contact, 'custom_fields').find(field => field.kind === name), 'value');
...
let getShippingStatus = contacts => {
  ...
  return Promise.all(statuses).then(statuses => {
    return contacts.map(contact => {
      let status = statuses.find(status => status.trackingNumber === contact.trackingNumber);
...
autopilot.get(`/smart_segments/contactlist_${smartSegmentId}/contacts/`)
